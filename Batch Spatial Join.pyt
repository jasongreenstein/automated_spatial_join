...
Purpose: script for python toolbox
        ...
        try:
            ...
            for fc in features:
                if arcpy.Describe(fc).shapetype == 'Polygon':
                    ...
                    fc = change_spatial_reference(fc)
                    arcpy.AddMessage('change spatial reference complete')

                    fc = delete_join_fields(fc,fields_to_delete)
                    fc = change_join_field_names(fc)
                    ...
                    i += 1
                    numProcessed = i-1  #number of geo. features joined
                    arcpy.AddMessage(f"{numProcessed}/{numFeatures} joins complete")
